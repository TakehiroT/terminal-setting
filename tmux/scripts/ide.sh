...
# === Window 5: Monitor (plan監視 + ビルド/テスト監視) ===
...
tmux split-window -t "$SESSION_NAME:Monitor" -v -l 40%
tmux select-pane -t "$SESSION_NAME:Monitor.2" -T "watcher"
# watcher paneは空のまま（ユーザーがビルド/テストコマンドを実行）
