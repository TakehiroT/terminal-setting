...
```
┌──────────────────────────────────────────────────────────────────────┐
│  Code  │  Git  │  Diff  │  Impl  │  Monitor  │         (タブバー) │
├──────────────────────────────────────────────────────────────────────┤
│                                                                │
│   Yazi (ビルトインプレビュー付き)                               │
...
| タブ | キー | 内容 |
|------|------|------|
| Code | `Alt+1` | Yazi（ビルトインプレビュー） + Terminal |
| Git | `Alt+2` | lazygit（delta でシンタックスハイライト付きdiff） |
| Diff | `Alt+3` | mainとの差分表示 + worktree切り替え |
| Impl | `Alt+4` | Claude Orchestrator + Codex Reviewer |
| Monitor | `Alt+5` | Plan Viewer + ビルド/テスト監視 |
...
## Monitor タブ（Vibe Coding監視）

planモード出力のリアルタイム監視とビルド/テスト監視用のタブ。
...
│   Watcher (40%)                                         │
│   ・ビルドやテストの監視用ターミナル                      │
│   ・例: pnpm run dev, npm run watch など                │
│                                                         │
└─────────────────────────────────────────────────────────┘
```
...
1. `Alt+5` でMonitorタブに移動
2. 上部ペインでplanファイルを自動監視
3. 下部ペインでビルド/テストコマンドを実行

### カスタマイズ

watcherペインで実行するコマンド例:
...
| キー | 動作 |
|------|------|
| `Alt+1/2/3/4/5` | タブ切り替え (Code/Git/Diff/Impl/Monitor) |
